 ...
 *  
 *  Personal Comments:
 *  I created this Unrar32 wrapper class for personal use 
 *  after running into a number of issues trying to use
 *  another COM Unrar32 product via COM interop.  I hope it 
...
namespace Schematrix
{
	...
	/// <summary>
	/// Wrapper class for Unrar32 DLL supplied by RARSoft.  
	/// Calls Unrar32 DLL via platform invocation services (pinvoke).
	/// DLL is available at http://www.rarlab.com/rar/Unrar32DLL.exe
	/// </summary>
	public class Unrar32 : IDisposable
	{
		#region Unrar32 DLL enumerations
		...
		#region Unrar32 DLL structure definitions
		...
		#region Unrar32 function declarations
		...
		[DllImport("Unrar32.dll")]
		private static extern void RARSetCallback(IntPtr hArcData, Unrar32Callback callback, int userData);

		[DllImport("Unrar32.dll")]
		private static extern void RARSetPassword(IntPtr hArcData,
			[MarshalAs(UnmanagedType.LPStr)] string password);

		// Unrar32 callback delegate signature
		private delegate int Unrar32Callback(uint msg, int UserData, IntPtr p1, int p2);
		...
		private Unrar32Callback callback=null;
		...
		public Unrar32() 
		{
			this.callback=new Unrar32Callback(RARCallback);
		}

		public Unrar32(string archivePathName) : this()
		{
		...
		~Unrar32()
		{
			...
			{
				Unrar32.RARCloseArchive(this.archiveHandle);
		...
		public void Close()
		{
			...
			// Close archive
			int result=Unrar32.RARCloseArchive(this.archiveHandle);
		...
		public void Open(string archivePathName, OpenMode openMode)
		{
			...
			// Open archive
			handle=Unrar32.RAROpenArchiveEx(ref openStruct);
			...
			// Set callback
			Unrar32.RARSetCallback(this.archiveHandle, this.callback, this.GetHashCode());
			...
			if(this.password.Length!=0)
				Unrar32.RARSetPassword(this.archiveHandle, this.password);
		...
		public bool ReadHeader()
		{
			...
			// Read next entry
			currentFile=null;
			int result=Unrar32.RARReadHeaderEx(this.archiveHandle, ref this.header);
		...
		public void Skip()
		{
			int result=Unrar32.RARProcessFile(this.archiveHandle, (int)Operation.Skip, string.Empty, string.Empty);
		...
		public void Test()
		{
			int result=Unrar32.RARProcessFile(this.archiveHandle, (int)Operation.Test, string.Empty, string.Empty);
		...
		{
			int result=Unrar32.RARProcessFile(this.archiveHandle, (int)Operation.Extract, destinationPath, destinationName);
